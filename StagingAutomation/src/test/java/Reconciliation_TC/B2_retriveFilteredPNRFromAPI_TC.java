...
import utilities.retrivePnrUtilis;
 ...
 * - Saves results into a CSV file inside /output
 */
public class B2_retriveFilteredPNRFromAPI_TC {

    // ✅ Logger
    private static final Logger log = LoggerFactory.getLogger(B2_retriveFilteredPNRFromAPI_TC.class);

    // ✅ Get Base URL from retrivePNR.json
    private static final String BASE_URL = retrivePnrUtilis.getBaseUrl();

    // ✅ XPath locator prefix
    private final String locate = "Envelope.Body.UniversalRecordSearchRsp.UniversalRecordSearchResult.";

    // ✅ Countries to loop through
    private static final String[] COUNTRIES = {"UAE", "EGY", "KSA"};

    // ✅ Set to ensure ProviderLocatorCode values are unique (no duplicates)
    private final Set<String> globalSeenProviderCodes = new HashSet<>();

    /**
     * 🔹 Helper Method
     * ✅ Ensures provider code is unique across the whole file.
     *
     * @param providerCode ProviderLocatorCode from API response
     ...
     * 4. Parse response
     * 5. Collect results and save them into a CSV file
     *
     * @param dbExcelPath Path to DB Excel file (output of DatabaseTest)
     * @return Path to output API results CSV file
     */
    public String processRecordsFromExcel(String dbExcelPath) {
        ...
        try (FileInputStream fis = new FileInputStream(dbExcelPath);
            ...
            for (int r = 1; r < rowCount; r++) {
                ...
                // ✅ Loop over all countries from retrivePNR.json
                for (String country : COUNTRIES) {
                    ...
                    if (response.getStatusCode() != 200) {
                        log.error("❌ Request failed for {} with status: {}", country, response.getStatusCode());
                        continue;
                    }
            ...
            if (!dir.exists() && dir.mkdirs()) {
                log.info("📂 Created output directory at {}", dir.getAbsolutePath());
            }

            // ✅ Save results into new CSV file
            String timestamp = new SimpleDateFormat("yyyyMMdd_HHmmss").format(new Date());
            outFile = System.getProperty("user.dir") + "/output/API_Results_" + timestamp + ".csv";
            CSVUtils.saveAsCSV(outFile, results);

            log.info("✅ API results saved to CSV: {}", outFile);
     ...
     * @param firstName    Traveler first name
     * @param lastName     Traveler last name
    ...
    /**
     * ✅ Parse API XML response and collect results.
     *
     * @param xmlPath   XML response parsed with XmlPath
     * @param pnr       PNR from DB Excel
     * @param credential Credential used (UAE, EGY, KSA)
     ...
     * @param columnName  Name of the column in header row
     * @return Value of the cell, or "" if not found
     */
