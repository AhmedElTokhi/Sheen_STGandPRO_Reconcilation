...
public class A1_GetGalileoPNR_TC {
    ...
    public void fetchAndSaveRecordsByCredential() throws IOException {
        ...
        InputStream inputStream = getClass().getClassLoader().getResourceAsStream("GalileoApi.json");
        if (inputStream == null) {
            throw new IOException("❌ Could not find GalileoApi.json in resources folder!");
        }
        ...
        String csvFilePath = outputDir + "/provider_codes_by_credential.csv";
            ...
            for (JsonNode cred : config.get("credentials")) {
                ...
                for (int i = 0; i <= 100000; i += 98) {
                    ...
                    for (int idx = 0; idx < results.size(); idx++) {
                        // ✅ Use correct XML path per index
